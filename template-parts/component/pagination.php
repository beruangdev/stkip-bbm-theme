<div class="mt-6 flex justify-between items-center gap-3">
  ...
  <div class="flex gap-3">
    ...
    for ($i = $start_page; $i <= $end_page; $i++) {
      $class = $i === $current_page ? 'bg-primary-700 hover:bg-primary-900 text-white' : 'text-gray-700 hover:bg-gray-300 dark:hover:bg-background-950';
      echo '<a href="' . get_pagenum_link($i) . '" class="px-3 py-1 rounded-md no-underline ' . $class . '">' . $i . '</a>';
    }

    if ($end_page < $total_pages) {
      echo '<a href="' . get_pagenum_link($total_pages) . '" class="px-3 py-1 rounded-md text-gray-700 hover:bg-gray-300 dark:hover:bg-background-950 no-underline">' . $total_pages . '</a>';
    }
    ?>
  </div>
